...
import epsilon.core.ParseException;
...
public class Parser {
    ...
    private FunctionStatement createFunction(boolean isAnonymous) {
        ...
        Token name = null;
        List<Token> parametersId = new LinkedList<>();
        List<Token> parametersType = new LinkedList<>();
        ...
        return new FunctionStatement(name, parametersId, parametersType, body, returnType);
    }
    ...
    private Statement createLetDeclaration() {
        ...
        if (match(Kind.Colon)) {
            type = consume(Kind.Identifier, "Expected declaring type after ':'.");
        }
